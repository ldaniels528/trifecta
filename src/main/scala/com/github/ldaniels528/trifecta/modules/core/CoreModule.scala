...
class CoreModule(config: TxConfig) extends Module {
  ...
  override def getInputSource(url: String): Option[MessageInputSource] = {
    url match {
      ...
      case s if s.startsWith("file:json:") => url.extractProperty("file:json:") map (JSONFileMessageInputSource(_))
      case s if s.startsWith("file:text:") => url.extractProperty("file:text:") map (TextFileMessageInputSource(_))
      case s if s.startsWith("http:") => Option(new HttpMessageInputSource(url))
      case _ => None
    }
  }
  ...
  override def getOutputSource(url: String): Option[MessageOutputSource] = {
    url match {
      ...
      case s if s.startsWith("file:json:") => url.extractProperty("file:json:") map (JSONFileMessageOutputSource(_))
