...
import com.ldaniels528.verify.vscript.VScriptCompiler
 ...
 */
class VxRuntimeContext(val config: VxConfig, val zkProxy: ZKProxy) {
  private val logger = LoggerFactory.getLogger(getClass)
  private implicit val scope = config.scope

  // create the result handler
  private val resultHandler = new VxResultHandler(config)
