...
class KafkaModule(rt: VxRuntimeContext) extends Module with BinaryMessaging with AvroReading {
  ...
  override def getCommands: Seq[Command] = Seq(
    ...
    Command(this, "kcount", countMessages, SimpleParams(Seq("field", "operator", "value"), Seq.empty), help = "Counts the messages matching a given condition"),
    ...
    Command(this, "kfindone", findOneMessage, SimpleParams(Seq("field", "operator", "value"), Seq.empty), "Returns the first occurrence of a message matching a given condition"),
