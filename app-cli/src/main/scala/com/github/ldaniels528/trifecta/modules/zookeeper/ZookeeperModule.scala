...
import com.github.ldaniels528.trifecta.io.zookeeper.{ZKProxy, ZookeeperMessageOutputSource}
...
class ZookeeperModule(config: TxConfig) extends Module {
   ...
   */
  override def getOutputSource(url: String): Option[ZookeeperMessageOutputSource] = {
    url.extractProperty("zk:") map (new ZookeeperMessageOutputSource(zk, _))
  }
