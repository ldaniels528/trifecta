...
case class KafkaRestFacade(config: TxConfig, zk: ZKProxy) {
  ...
  def getBrokerDetails = Future {
    KafkaMicroConsumer.getBrokerList.groupBy(_.host) map { case (host, details) => BrokerDetailsJs(host, details) }
  }
  ...
  def getZkData(path: String, format: String) = {
    ...
    Future {
      ...
      case Some(Success(js: JValue)) => FormattedData(`type` = JSON, compactRender(js))
...
object KafkaRestFacade {
  ...
  case class BrokerDetailsJs(host: String, details: Seq[BrokerDetails])
